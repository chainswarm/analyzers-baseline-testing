...
from analyzers_baseline.scripts.run_pipeline import parse_path_metadata, construct_output_path
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run feature analysis on blockchain transaction data"
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    setup_logger(f"analyzers-baseline-{network_hint}-features")
    ...
    logger.info(f"Feature analysis complete: {result['features_count']} features computed")
